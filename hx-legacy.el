...
(defvar battle-haxe-yasnippet-completion-expansion nil
  "When non-nil, battle-haxe will expand completions using yasnippet when
available.")
...
(define-minor-mode battle-haxe-mode
  ...
  (progn
    ...
    ;; (set (make-local-variable 'eldoc-documentation-function) #'eldoc-documentation-compose)
    (add-hook 'eldoc-documentation-functions #'battle-haxe-eldoc-function nil t)
    )
...
(defun battle-haxe-company-backend (command &optional arg &rest ignored)
  "'company-mode' completion back-end using Haxe compiler services. Backend is
called using COMMAND, ARG, and the rest is IGNORED."
...
(defun battle-haxe-eldoc-function (callback &rest _args)
...
such as @position."
  (rx-let ((any-ws (zero-or-more (syntax whitespace)))
    ...
    (let ((re (rx any-ws (group (while-not ":")) ":"
                  (group (while-not ":")) ":" any-ws
                  (or "character" "characters") any-ws
                  (group (zero-or-more digit))
                  (optional "-" (zero-or-more digit)))))
      (cl-mapcar #'cons '(file line char) (-drop 1 (s-match re pos-string)))))
  )
